...
let { data_suplier, data_barang,data_transaksi_detail_barang} = storeToRefs(barang_masuk_store)
...
let show_suplier_drop = ref(false)
...
let clickItemSuplier = (item)=>{
    supplier.value = item.name
...
watch(supplier,async()=>{
    ...
    barang_masuk_store.getDataSuplier(supplier.value)
...
onMounted(async()=>{
    barang_masuk_store.getDataSuplier()
...
let removeBarangDetail = (index)=>{
    barang_masuk_store.removeDetailTransakiBarangMasuk(index)
    qty.value = 0
    harga.value = 0
    single_data_product.value = {}
    barang.value = ''
...
let saveTransaksiMasuk = ()=>{
    
    if(id_supplier.value === ''){
        show_msg.value = true
        content_msg.value = 'Seleksi Supplier Diperlukan'
    }else if(data_transaksi_detail_barang.value.length <= 0){
        show_msg.value = true
        content_msg.value = 'Detail Barang Minimum 1 Barang'
    }else{
        barang_masuk_store.saveTransaksiMasuk(id_supplier.value,date.value,deskripsi.value).then(x=>{
            ...
            barang_masuk_store.clearDetailTransakiBarangMasuk()
...
<template>
    <div class=" flex flex-row justify-between items-center mb-4 ">
        <div>
            <h1 class="text-2xl text-[#2d354f] ">Tambah Transaksi Barang Masuk</h1>
        </div>
        <div>
        ...
        <div class="grid grid-cols-4 gap-4 ">
            <div class="relative ">
                    <div class="flex flex-col gap-1">
                        <lable class="text-md">Seleksi Suplier</lable>
                        <input @click="show_suplier_drop = true" @blur="blurAwait" v-model="supplier" type="text" placeholder="Cari Nama Atau Email Suplier" class=" px-2 py-1 text-md bg-[#e4edff] border-gray-300 rounded-lg focus:outline-none focus:ring-1 focus:ring-[#2563EB] focus:border-[#2563EB]">
                    </div>
                    <div v-show="show_suplier_drop" class="z-10 absolute bg-[#e4edff] rounded-lg w-full mt-2 py-2 overflow-y-auto h-[250px]">
                        <li v-for="item in data_suplier" :key="item.id" @click="clickItemSuplier(item)" class="list-none px-4 hover:bg-[#c6d9ff] cursor-pointer py-2">{{ item.name }} - {{ item.email }}</li>
                        <li v-if="data_suplier.length <= 0" class="list-none px-4 hover:bg-[#c6d9ff] cursor-pointer py-2">Supplier Not Found</li>
                    </div>
            </div>
            <div>
                    <div class="flex flex-col gap-1">
        ...
        <div class="shadow rounded-2xl p-4 mt-4 border-1 border-gray-200">
            ...
            <div class="grid grid-cols-4 gap-4 items-end">
                ...
                <div>
                    ...
                    <div>
                        <div class="flex flex-col  gap-1">
                            <lable class="text-md">Harga</lable>
                            <input v-model="harga" type="number" min="0" placeholder="Input Nama Barang" class=" px-2 py-1 text-md bg-[#e4edff] border-gray-300 rounded-lg focus:outline-none focus:ring-1 focus:ring-[#2563EB] focus:border-[#2563EB]">
                        </div>
                </div>
