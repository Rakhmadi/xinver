...
let export_ = async()=>{
    laporan_state.getDataLaporanBarangMasuk(start_date.value,end_date.value).then(async()=>{

        let data_export = laporan_state.data_laporan_barang_masuk.map(x=>{
            return {
                kode_transaksi : x.code_transaction,
                tanggal_transaksi : x.date, 
                deskripsi : x.description,
                nama_supplier : x.name,
                total_qty : x.sum_qty_detail,
                total_jenis_barang : x.product_count,
                total_harga : x.all_total_price
...
// buat worksheet
        ...
        const filePath = await save({
            defaultPath: `laporan_barang_masuk_${timestamp}.xlsx`,
            filters: [
            {
                name: "Excel",
                extensions: ["xlsx"]
