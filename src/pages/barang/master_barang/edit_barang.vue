...
onMounted(()=>{
    ...
    data_barang.getSingleDataBarang(Number(route.params.id)).then(x=>{
        name.value = data_barang.single_data_barang.name
        SKU.value = data_barang.single_data_barang.sku
        category_id.value = data_barang.single_data_barang.category_id
        merek_id.value = data_barang.single_data_barang.merk_id
        location_id.value = data_barang.single_data_barang.location_id
        jenis_unit.value = data_barang.single_data_barang.unit
        stock_awal.value = data_barang.single_data_barang.stock
        harga.value = data_barang.single_data_barang.price
        description.value = data_barang.single_data_barang.description
    }).catch((err)=>{
        console.log(erre)
    })
...
let update = async()=>{
    if(name.value.length <= 5){
        ...
        let exec_data_barang = await data_barang.updateDataBarang(name.value,
        SKU.value,
        category_id.value,
        merek_id.value,
        location_id.value,
        ...
        jenis_unit.value,
        stock_awal.value,
        harga.value,data_barang.single_data_barang.id)
...
<template>
    <h1 class="text-xl text-[#2d354f] font-semibold mb-4">Edit Data Barang "{{ data_barang.single_data_barang.code_barang }}" dengan SKU "{{  data_barang.single_data_barang.sku  }}"</h1>
    ...
    <form>
        ...
        <div class=" flex flex-row gap-2">
            <button @click="router.push(`/barang/detail_barang/${data_barang.single_data_barang.id}`)" class="bg-[#2563EB] mt-6 no-underline text-sm flex flex-row items-center rounded-full py-1 px-3 gap-1 border-1 border-transparent hover:bg-[#1E40AF] focus:border-b-1 text-white cursor-pointer">
                <div>
                ...
                <button @click.prevent="update" class="bg-[#2563EB] mt-6 no-underline text-sm flex flex-row items-center rounded-full py-1 px-3 gap-1 border-1 border-transparent hover:bg-[#1E40AF] focus:border-b-1 text-white cursor-pointer">
                    <div>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-4">
